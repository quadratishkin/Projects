...
let oldValue = '';
document.querySelector('.search-wrapper__search-btn').addEventListener('click', ()=>{
    const searchEl = document.getElementById('search-text'); // значение строки поиска
    const searchText = searchEl.value.trim();
    const newColumnsData = Array.from(document.getElementsByClassName('column'));
    ...
    if (!searchText.length) {
        ...
        return; // если повторно ищем, то нам не надо делать алгоритм заново.
    }
    ...
    activeFilters.forEach(filterName => {
        const activeColumn = newColumnsData.find(column => { // Ищем в массиве колонок ту, которая соответствует фильтру
            return  column.children[0].textContent === filterName;
            }
        );
        const cells = Array.from(activeColumn.children);
        cells.forEach(cell => { // forEach, т.к. надо всё равно пройтись по всем клеткам. Нет смысла проходить через for of.
            const foundMatchesCount = (cell.textContent.match(searchRegExp) || []).length;
            if (foundMatchesCount === 0) { // нет совпадений в ячейке? скипаем!
                return;
            }
...
function resetHighlight(searchText) {
    ...
    const matches = Array.from(document.getElementsByClassName('search-text__highlighted'));
